...
async function main() {
  ...
  assert(uiHtml.includes("conversationRunning && effectiveInputMode === 'guide'") && uiHtml.includes("effectiveInputMode === 'next' && !opts.fromQueue") && uiHtml.includes('[Queue] Current turn is locked; prompt will run after it completes.'), 'ui html: Guide steers active turns while Next queues future work');
