...
class FirstInputTitleAndNetworkRecoveryContractTest {
    ...
    fun firstInputUsesOneIndependentTitleRequestWithoutDirectCopy() {
        ...
        assertTrue(compactSource.contains("requestFirstInputConversationTitle( targetConversationId, firstTitleMessageId, titleInput, titleConfig, turnIntelligence, turnThinkingTierMap, onFailure = { lastTitleFailure = it }, )"))
        assertTrue(titleApplicationSource.contains("requestAndApplyFirstInputTitle("))
        assertTrue(titleApplicationSource.contains("commitFirstInputConversationTitle("))
        assertTrue(titleApplicationSource.contains("conversationStore.save(updated)"))
        assertTrue(titleRequestSource.contains("applyTitle: (String) -> Result<Boolean>"))
