...
async function main() {
  ...
  assert(uiHtml.includes("effectiveStatus !== 'available' && effectiveStatus !== 'unvalidated' && effectiveStatus !== 'verified' && effectiveStatus !== 'degraded'") && agentKernelSource.includes("validationStatus === 'verified' || validationStatus === 'degraded'"), 'model selectors: keep fixed unvalidated models visible while accepting effective Standard verified or degraded evidence');
