...
async function main() {
  ...
  assert(mainSource.includes('const singleInstanceLock = app.requestSingleInstanceLock()') && mainSource.includes("app.on('second-instance'") && mainSource.includes('createDesktopWindow = () =>') && mainSource.includes('BrowserWindow.getAllWindows().filter'), 'main process: repeated desktop launches route into one shared backend process and create additional windows instead of duplicate runners');
  ...
  assert(mainTs.includes('function hasPortableRootState') && mainTs.includes('function canWriteDirectory') && mainTs.includes('function isProtectedInstallRoot') && mainTs.includes("process.env.ProgramFiles") && mainTs.includes("return app.getPath('userData')") && mainTs.includes('const explicitRoot = pathArgValue(args,') && mainTs.includes('firstRunInit(root);') && mainTs.includes('firstRunInit(fallbackRoot)') && mainTs.includes('logStartupFailure(`firstRunInit:${root}`'), 'main startup: packaged default root falls back from protected or unwritable install directories to userData and logs initialization failures');
  assert(mainTs.includes('existingPcId = fs.existsSync(pcHashPath)') && mainTs.includes('if (existingPcId !== pcId) fs.writeFileSync(pcHashPath, pcId') && memoryLabTs.includes('if (!fs.existsSync(this.indexPath))') && memoryLabTs.includes('this.normalizeIndex(raw);') && !memoryLabTs.includes('else this.saveIndex(this.normalizeIndex(this.loadIndex()))'), 'startup initialization: avoids rewriting PC_Hash.config and Memory Lab index on every launch');
  ...
  assert(mainTs.includes('const syncAutomationWakeSoon = () =>') && mainTs.includes('syncAutomationWakeSoon();') && mainTs.indexOf('mainWindow = createDesktopWindow();') < mainTs.indexOf("app.on('will-quit'"), 'main startup: desktop window is created before noncritical automation wake sync can block startup');
