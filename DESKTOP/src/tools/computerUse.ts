...
async function stopTakeoverOverlay(): Promise<void> {
  ...
  const script = [
    ...
    "$overlayPattern = '(?i)(^|\\s)-File\\s+\"?[^\"\\s]*takeover-overlay-[^\"\\s]*\\.ps1\"?'",
    "$selfPid = $PID",
    "$targets = Get-CimInstance Win32_Process | Where-Object { $_.ProcessId -ne $selfPid -and $_.Name -match '^powershell(\\.exe)?$' -and $_.CommandLine -match $overlayPattern }",
...
async function startTakeoverOverlay(durationMs = 0, input: { colors?: string[]; speed?: number; width?: number; ownerPid?: number } = {}): Promise<Record<string, unknown>> {
  ...
  const script = [
    ...
    'public class NewmarkOverlayForm : Form {',
    '  public NewmarkOverlayForm() {',
...
function stringifyComputerUseResult(result: Record<string, unknown>): string {
  ...
  return Buffer.byteLength(output, 'utf8') <= 32768
    ...
    : JSON.stringify({
      ...
      capture_max_width: result.capture_max_width,
      capture_max_height: result.capture_max_height,
      image_width: result.image_width,
      image_height: result.image_height,
      image_bytes: result.image_bytes,
      image_mime: result.image_mime,
      screenshot_retention: result.screenshot_retention,
