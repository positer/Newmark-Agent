...
class MobileEmptyResponseRetryContractTest {
    ...
    fun agentLoopRetriesFiveTimesOnlyAfterExplicitEmptyResponses() {
        ...
        val titleSource = File("src/main/java/com/newmark/mobile/vm/FirstInputTitleRequest.kt").readText()
        assertTrue(Regex("apiClient\\.chat\\(").findAll(source).count() == 1)
        assertTrue(Regex("apiClient\\.chat\\(").findAll(titleSource).count() == 2)
        assertTrue(titleSource.contains("if (imageAttachmentCount > 0 && title.isBlank())"))
        assertTrue(source.contains("requestAndApplyFirstInputTitle("))
        ...
        assertTrue(source.contains("return Result.success(response.copy("))
        assertTrue(source.contains("[Network continuation]"))
        assertTrue(source.contains("networkRetryAttempt < 3"))
