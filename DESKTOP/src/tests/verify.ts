...
async function main() {
  ...
  await verifyEditorCompletion(uiHtml, assert);
  ...
  assert(uiHtml.includes("aria-label=\"' + escAttr(t('common.delete')) + '\"") && !uiHtml.includes("iconSvg('x', 'Remove model'"), 'ui html: model remove action uses localized title/icon label');
  ...
  assert(uiHtml.includes("var selected = state.memoryLabOverviewFocus === id")
    ...
    && uiHtml.includes('timestamp - last < minimumFrameMs')
    && uiHtml.includes('state.memoryLabOverviewFrameRunning = true')
    && uiHtml.includes('window.stepMemoryLabOverviewPhysics(timestamp)')
    && uiHtml.includes('window.paintMemoryLabOverviewFrame()')
    && uiHtml.includes('var fanout = Math.min(24')
    && uiHtml.includes('state.memoryLabOverviewRepulsionOffset')
    && uiHtml.includes('svg.replaceChildren()')
    && uiHtml.includes("edgeElement.base.setAttribute('d', d)")
    && !uiHtml.includes('svg.innerHTML = paths')
    ...
    && uiHtml.includes('window.stopMemoryLabOverviewPipeline')
  ...
  assert(!overviewSelectSource.includes('loadMemoryLab')
    ...
    && !componentSelectSource.includes('api.')
    && visualizationLoadReasons.join(',') === 'open,reindex,reset',
  'ui html: Memory Lab loads one visualization snapshot only for open/reset/reindex while overview/detail clicks remain memory-only');
  ...
  assert(modelValidationGuard >= 0
    && modelValidationStart >= 0
    && uiHtml.indexOf("t('model.validationRequiresBaseModel')", modelValidationGuard) < modelValidationStart
  ...
  assert(uiHtml.includes("events[priorEventIndex] = Object.assign({}, priorEvent, { completed: true, displayImage: rawEvent.displayImage || priorEvent.displayImage })")
    ...
    && uiHtml.includes("if (run.userToggled !== true) run.expanded = true"), 'ui html: completed tools update their original row, live Build blocks expand, and terminal blocks auto-collapse unless manually toggled');
  ...
  assert(uiHtml.includes('window.submitCurrentAction = function(source)')
    ...
    && uiHtml.includes('animation: marquee-rotate var(--marquee-speed) linear infinite;'), 'ui html: current running conversation keeps responsive input through bounded large-stream rendering, 10fps Build DOM backpressure, immediate terminal updates, and a live GPU-composited marquee border');
  ...
  assert(uiHtml.includes('--modal-surface: rgba(248,250,255,0.82);') && uiHtml.includes('--modal-shadow:') && uiHtml.includes('brightness(1.01)') && !uiHtml.includes('brightness(1.12)'), 'ui light theme: carrier-glass sub-windows retain restrained frost without the removed dedicated foreground glow');
  assert(uiHtml.includes('[data-theme="light"] #submit-btn.running-action') && uiHtml.includes('background: rgba(255,255,255,0.72)') && uiHtml.includes('--marquee-speed: 3s;') && uiHtml.includes('--marquee-width: 2px;') && uiHtml.includes('--g1: #000000;') && uiHtml.includes('--g2: #ffffff;') && uiHtml.includes('--g3: #000000;') && uiHtml.includes('--g4: #ffffff;') && !uiHtml.includes('window.setGradientColor') && !uiHtml.includes('window.setGradientSpeed') && !uiHtml.includes('window.setGradientWidth'), 'ui light theme: submit/stop surface is light while marquee palette, speed, and width are fixed without customization controls');
  ...
  assert(modeConversationStressSource.includes('Flow A running')
    ...
    && modeConversationStressSource.includes('six concurrent conversation Flows never share target records')
    && modeConversationStressSource.includes('conversation B tracking expired after 5 minutes')
    && modeConversationStressSource.includes('load-earlier clamps at zero')
  ...
  assert(uiHtml.includes('editorPredictionEnabled: true') && uiHtml.includes('window.toggleEditorPrediction') && uiHtml.includes('window.scheduleEditorCompletion') && uiHtml.includes('}, 300);') && uiHtml.includes('beforeStart = Math.max(0, pos - 3200)') && uiHtml.includes('afterEnd = Math.min(ta.value.length, pos + 800)') && uiHtml.includes('class="editor-ghost"') && uiHtml.includes('window.highlightEditorCode(combined, language)') && uiHtml.includes('editor-ghost-text') && uiHtml.includes("state.editorCompletionText && e.key === 'Tab'"), 'ui html: Copilot prediction defaults on, retriggers after an idle window, sends bounded cursor-local context, reflows pale insertion text, and accepts with Tab');
  ...
  assert(mainSource.includes("completion: true, preferCopilot: true") && mainSource.includes('editorCompletionControllers.get(ownerId)?.abort') && mainSource.includes('agent:editorCompletionDelta') && preloadSource.includes('editorCompleteCancel') && preloadSource.includes('onEditorCompletionDelta') && agentSourceForEditor.includes("model.provider_protocol === 'github_models'") && agentSourceForEditor.includes("'chat_stream'") && agentSourceForEditor.includes('EDITOR_COMPLETION_MAX_TOKENS = 96') && agentSourceForEditor.includes('EDITOR_COMPLETION_TIMEOUT_MS = 6500') && agentSourceForEditor.includes('chatStreamWithTools'), 'editor prediction: prefers a usable Copilot, forces streaming for early text, cancels superseded requests, and uses a bounded low-latency completion budget');
  ...
  assert(uiHtml.includes('function messageActionsHtml(role, text, messageIndex)') && uiHtml.includes('window.copyMessageText = async function(button)') && uiHtml.includes('window.editUserMessage = function(button, messageIndex)') && uiHtml.includes('window.submitUserMessageEdit = async function') && uiHtml.includes('editBranchApi.branchConversation(target, Number(messageIndex), text, {') && uiHtml.includes("clientMessageId: String(message") && uiHtml.includes("data-run-id=\"' + escAttr(String(guide.runId || event.runId || ''))") && uiHtml.includes('message._newmarkMessageTarget') && uiHtml.includes('message-inline-editor') && !uiHtml.includes("addMsg('assistant', '[Error] ' + ((result && result.error) || 'Unable to edit message.')"), 'ui html: user and Guide messages retain stable target/run identity, edit inline, create a durable branch, and report failures outside the chat timeline');
  ...
  const editSubmitSource = uiHtml.match(/window\.submitUserMessageEdit = async function[\s\S]*?\n\};/)?.[0] || '';
  assert(uiHtml.includes('async function stopTargetConversationForMessageEdit(target)') && uiHtml.includes("api.stopConversation({ target: target, runId: runId, force: false })") && uiHtml.includes("api.stopConversation({ target: target, runId: runId, force: true })") && editSubmitSource.indexOf('renderProvisionalEditedBranch(') >= 0 && editSubmitSource.indexOf('renderProvisionalEditedBranch(') < editSubmitSource.indexOf('stopTargetConversationForMessageEdit(target)') && uiHtml.includes('restoreProvisionalEditedBranch(provisionalSnapshot, target)') && uiHtml.includes("delete state.activeSendCallsByTarget[runtimeKey]") && uiHtml.includes('optimisticMessageIndex: Number(messageIndex)') && uiHtml.includes('renderConversationBranchPagers();'), 'ui html: edit submit immediately renders the isolated new page before stopping only the selected conversation, and rolls back if persistence fails');
  assert(uiHtml.includes('function renderProvisionalEditedBranch(message, messageIndex, editedText, target, editingGuide)') && uiHtml.includes("if (!editingGuide) return [];") && uiHtml.includes('copiedRun.events = guideIndex >= 0 ? events.slice(0, guideIndex)') && uiHtml.includes("type: 'guide_applied'") && uiHtml.includes('var normalizedMessages = cacheConversationMessages(messages, renderTarget)'), 'ui html: provisional message edits remove the owning Build immediately while Guide edits preserve only the owning Build prefix and replace the Guide node in place');
  assert(uiHtml.includes("normalizedName === 'web_fetch'") && uiHtml.includes("return 'web_fetch · '") && uiHtml.includes("normalizedName === 'web_search'") && uiHtml.includes("return 'web_search · '"), 'ui html: web_fetch and web_search activity labels expose their concrete URL or query target');
  assert(uiHtml.includes("searched_web") && uiHtml.includes("搜索了网页") && uiHtml.includes("fetched_web") && uiHtml.includes("抓取了网页") && uiHtml.includes("web_catch"), 'ui html: web_search reports searched_web while web_fetch/web_catch report fetched_web tags');
  assert(uiHtml.includes('guideMessageIndexByClientId: {}') && uiHtml.includes("messageActionsHtml('user', content, -1)") && !uiHtml.includes("messageActionsHtml('user', content, indexed ? messageIndex : -1)") && uiHtml.includes('function hydrateWorkRunGuideMessages(root, run)') && agentSourceForEditor.includes("status: 'interrupted' as const") && agentSourceForEditor.includes('run.events.slice(0, eventCut)') && agentSourceForEditor.includes('run.guides.slice(0, guideCut)') && agentSourceForEditor.includes('const branchPageReplacement = !!incoming.branchReset || !!incoming.tree || !!incoming.branches?.length;') && agentSourceForEditor.includes('preferred.workRuns = branchPageReplacement'), 'ui/core: every Guide keeps one-click copy but is no longer editable in the timeline, and every branch page replaces rather than merges its transcript and Build tree');
  ...
  assert(conversationKernelSource.includes("type: 'queue_update'") && conversationKernelSource.includes('queueItems: this.queueItems(runtime.target)') && conversationKernelSource.includes('queuePaused: runtime.queuePaused === true'), 'kernel: queue_update work events export structured queueItems ids + paused so PC UI and mobile mirror the same rows');
  ...
  assert(uiHtml.includes('modelHasResponseAnomaly(modelEntry)') && uiHtml.includes('modelEntry.enabled === false') && !uiHtml.includes("effectiveStatus !== 'available' && effectiveStatus !== 'unvalidated'"), 'model selectors: response observations remain warnings and only user-disabled models are hidden');
  const modelValidationSource = fs.readFileSync(path.join(process.cwd(), 'src', 'core', 'modelValidation.ts'), 'utf-8');
  assert(agentKernelSource.includes('const service = new ModelValidationService({') && agentKernelSource.includes('auditSink: (event: ValidationAuditEvent)') && agentKernelSource.includes('await p.modelCatalog()') && agentKernelSource.includes('createProviderValidationAdapter(p, m.name)') && agentKernelSource.includes('visionChallenge: declaredVision') && modelValidationSource.includes("'strict_json'") && modelValidationSource.includes("'unknown_tool_exclusion'") && modelValidationSource.includes("'tool_result'") && modelValidationSource.includes('validateImageOutput('), 'model validation: uses the shared Standard/Extended service for catalog hypotheses, per-check progress, and real text, stream, JSON, tool, vision, and image-byte probes');
  ...
  assert(uiHtml.includes('var activation = api.activateConversation ? api.activateConversation(target)')
  ...
  assert(agentKernelRunnerTs.includes('function providerSessionIdentity(agent: Agent): string')
    && agentKernelRunnerTs.includes('agent.activeWorkRunBranchId()')
    && agentKernelRunnerTs.includes('branchConversationIdentity(conversationId, runBranchId)')
    && agentKernelRunnerTs.includes('providerSessionIdentity(currentAgent)'),
  'provider session cache: optional remote session identity is bound to the Build owner branch and never shared by sibling branches');
  ...
  assert(mainTs.includes("return agent.listArchivesAsync(scope === 'all' ? 'all' : 'workspace')") && mainTs.includes("return agent.readArchiveAsync(name)") && agentTs.includes('async listArchivesAsync(') && agentTs.includes('async readArchiveAsync(') && agentTs.includes('await new Promise<void>(resolve => setImmediate(resolve))') && agentTs.includes('async readArchiveFirstLine('), 'archives: listing and reading use async IPC with first-line-only scans and event-loop yields so large archive directories never block the main process');
  ...
  const crossPlatformScript = fs.existsSync(path.join(process.cwd(), 'scripts', 'check-cross-platform-env.cjs')) ? fs.readFileSync(path.join(process.cwd(), 'scripts', 'check-cross-platform-env.cjs'), 'utf-8') : '';
  const harmonyDoc = fs.existsSync(path.join(process.cwd(), '..', 'HARMONYOS_INTEGRATION.md')) ? fs.readFileSync(path.join(process.cwd(), '..', 'HARMONYOS_INTEGRATION.md'), 'utf-8') : '';
  const macDoc = fs.existsSync(path.join(process.cwd(), '..', 'docs', 'macos-build.md')) ? fs.readFileSync(path.join(process.cwd(), '..', 'docs', 'macos-build.md'), 'utf-8') : '';
  assert(crossPlatformScript.includes('Build for macOS is supported only on macOS') === false
    && crossPlatformScript.includes('dist:mac script exists')
    && crossPlatformScript.includes("context.electronPlatformName !== 'win32'")
    && crossPlatformScript.includes('headless HTTP server source exists')
    && harmonyDoc.includes('HarmonyOS 集成') && harmonyDoc.includes('dist/server.js') && harmonyDoc.includes('dist/launcher.js')
    && macDoc.includes('macOS 主机') && macDoc.includes('npm run dist:mac'), 'cross-platform env: check script validates macOS/HarmonyOS headless build wiring and macOS host constraint, with docs for both targets');
  const harmonyBuildScript = fs.existsSync(path.join(process.cwd(), 'scripts', 'dist-harmonyos.cjs')) ? fs.readFileSync(path.join(process.cwd(), 'scripts', 'dist-harmonyos.cjs'), 'utf-8') : '';
  assert(harmonyBuildScript.includes("'dist-harmonyos'") && harmonyBuildScript.includes('newmark-server.bundle.cjs') && harmonyBuildScript.includes('harmonyos-manifest.json') && harmonyBuildScript.includes('http://127.0.0.1:47890/api/state') && harmonyBuildScript.includes('headless server HTTP smoke ok'), 'HarmonyOS headless build: dist-harmonyos.cjs packs the platform-independent core, bundles pure-JS deps, and HTTP-smokes the headless server');
  ...
  const releaseAllScript = fs.readFileSync(path.join(process.cwd(), 'scripts', 'release-all.cjs'), 'utf-8');
  ...
  assert(mainTs.includes('startupAttempt = 0') && mainTs.includes('startupWindow = createDesktopWindow(false, true)') && mainTs.includes("ipcMain.handle('startup:waitForBackend'") && mainTs.includes('runStartupPrewarmBarrier') && mainTs.includes('await Promise.all([startupShellReady, startupAgentReady])') && mainTs.includes('const reusesPreloadedUi = attemptOneNavigationPreloaded') && mainTs.includes('if (!reusesPreloadedUi)') && mainTs.includes('loadDesktopWindowUi(startupUiWindow, attemptId)') && mainTs.includes('waitForUiReadiness(startupUiWindow)') && mainTs.includes('promoteStartupUi(startupUiWindow)') && !mainTs.includes('createDesktopWindow!(true, false, attemptId)') && !mainTs.includes('shellWindow.destroy()') && !mainTs.includes('win.webContents.stop();'), 'main startup: shows a lightweight shell first, then navigates and atomically promotes the same hydrated window');
  assert(uiHtml.includes('window.history.replaceState(null, document.title, window.location.pathname + window.location.hash)') && mainTs.includes('CONFIG_RELOAD_RUNTIME_TIMEOUT_MS') && mainTs.includes("boundedOperation(") && mainTs.includes("win.webContents.on('render-process-gone'"), 'desktop resilience: config reload cannot retain the one-shot startup barrier or wait forever, and a lost renderer is recovered');
  ...
  assert(distLinuxScript.includes("['--linux', '--publish', 'never'") && distLinuxScript.includes('NEWMARK_RELEASE_OUTPUT_DIR'), 'Linux packaging: local artifact creation cannot implicitly publish or wait on release credentials');
  ...
  assert(mainTs.includes('startupWindow = createDesktopWindow(false, true)') && mainTs.includes('mainWindow = startupWindow') && mainTs.includes('createTray();') && mainTs.includes("tray.on('click', showMainWindow)") && mainTs.includes("tray.on('double-click', showMainWindow)") && mainTs.includes('if (tray) return;'), 'tray lifecycle: creates one tray with the lightweight startup window and reuses it while showing the promoted main window');
  ...
  assert(distLinuxScript.includes("replace(/\\\\/g, '/')") && distLinuxScript.includes("'wslpath', '-a', pathArgument"), 'Linux release build: no-space Windows artifact roots survive the WSL argv bridge without losing path separators');
  ...
  assert(releaseUiSmoke.includes('function seedDynamicI18nState') && releaseUiSmoke.includes("contextCompression: ''") && !uiHtml.includes('id="context-compression-pill"') && releaseUiSmoke.includes("nextQueue: '队列 1'") && releaseUiSmoke.includes("modelAuto: '自动'"), 'release ui smoke: validates dynamic language switching while keeping the removed compression placeholder absent');
  ...
  assert(uiHtml.includes('node.vx = ((node.vx || 0) + node.fx) * 0.72;')
    && uiHtml.includes('node.vy = ((node.vy || 0) + node.fy) * 0.72;')
    && uiHtml.includes('var fanout = Math.min(24')
    && !uiHtml.includes('for (var b = a + 1; b < graph.nodes.length; b++)'),
  'ui Memory Lab overview: bounded sampled repulsion retains damped tag motion without quadratic all-pairs work');
  assert(uiHtml.includes('window.memoryLabOverviewGraphSignature = function(index)')
    && uiHtml.includes("window.loadMemoryLabVisualization('open'")
    && uiHtml.includes("window.loadMemoryLabVisualization('reset'")
    && uiHtml.includes("window.loadMemoryLabVisualization('reindex'")
    && uiHtml.includes("window.selectMemoryLabOverviewNode('tag:' + tag);")
    && uiHtml.includes('renderPanTransform();')
    && uiHtml.includes('state.memoryLabOverviewFrameRunning')
    && uiHtml.includes('state.memoryLabOverviewNodeElements')
    && uiHtml.includes('state.memoryLabOverviewEdgeElements')
    && !uiHtml.includes('memoryLabOverviewPanEdgeX'),
  'ui Memory Lab overview: lifecycle snapshots own graph identity while a non-overlapping retained-DOM pipeline handles zoom and drag');
  ...
  assert(releaseRealProviderStress.includes("s.status === 'idle' && !s.goal && text.includes"), 'release real provider stress: Goal completion waits for the persisted Goal to clear instead of accepting a quoted future marker');
  ...
  assert(distPortableScript.includes("runBuilder(['--win', 'dir', '--publish', 'never']") && distPortableScript.includes("runBuilder(['--win', 'msi', '--prepackaged', unpackedDir, '--publish', 'never']") && distPortableScript.includes('verifyMsiInstaller()') && distPortableScript.includes('ELECTRON_BUILDER_CACHE') && distPortableScript.includes('.electron-builder-cache') && distPortableScript.includes('MSI installer and win-unpacked zip pack verified'), 'dist windows release: builds patched win-unpacked first, then packages MSI from the prepackaged directory without publishing during local packaging');
  assert(distPortableScript.includes('NEWMARK_RELEASE_OUTPUT_DIR') && releaseAllScript.includes('NEWMARK_RELEASE_OUTPUT_DIR') && distPortableScript.includes('--config.directories.output='), 'release packaging: one optional output root keeps Windows, Linux, and Android assets together when the default directory is externally locked');
  assert(distPortableScript.includes('verifyReleaseCliSmoke()') && distPortableScript.includes('release-cli-smoke.cjs') && distPortableScript.includes('NEWMARK_TEST_EXE: unpackedExe') && distPortableScript.includes('ensureNodePtyConptyAssets(nodePtyRoot)') && distPortableScript.includes("'conpty.dll', 'OpenConsole.exe'"), 'dist windows release: binds CLI smoke to the current unpacked artifact, restores node-pty ConPTY assets, and runs release CLI smoke after packaging');
  ...
  assert(mainTs.includes('if (!automationWakeMode)') && mainTs.includes('startupWindow = createDesktopWindow(false, true)') && mainTs.includes('if (automationWakeMode)') && mainTs.includes('await ensureStartupAutomation()') && mainTs.includes('await automation!.tick()') && mainTs.includes('automation!.stop();') && mainTs.includes('app.quit();') && mainTs.indexOf('await automation!.tick()') < mainTs.indexOf('scheduleDeferredDesktopStartup(startupUiWindow)'), 'main automation wake: skips the lightweight desktop shell, runs due schedules headless, and exits before deferred sidecar prewarm');
  ...
  assert(preloadTs.includes("refreshSkills: () => ipcRenderer.invoke('skills:refresh')") && preloadTs.includes("marketSkillSources: () => ipcRenderer.invoke('skills:marketSources')") && preloadTs.includes("memoryLabRead: (selector?: string) => ipcRenderer.invoke('memoryLab:read'") && preloadTs.includes("memoryLabVisualization: () => ipcRenderer.invoke('memoryLab:visualization')") && preloadTs.includes('updateCheckGithub') && preloadTs.includes('updateApplyGithub') && preloadTs.includes('terminalKill: (sessionId: string, timeoutMs?: number)'), 'preload: exposes skills refresh, market source management, one-shot Memory Lab visualization, updates, and terminal kill timeout');
  ...
  assert(mainTs.includes("if (!guideText || !runtime?.runId)")
    && mainTs.includes('let ConversationKernel decide whether that run can')
    && mainTs.includes('await ensureElectronUtilityPool().enqueueGuide(envelope)'),
  'mobile Guide hand-off: a finalization snapshot delegates current-run reactivation to the conversation kernel instead of rejecting on stale running=false');
  ...
  const computerUseSessionTs = fs.readFileSync(path.join(process.cwd(), 'src', 'core', 'computerUseSession.ts'), 'utf-8');
  assert(computerUseSessionTs.includes('computerUse occupied') && computerUseSessionTs.includes('enabledByRuntime') && computerUseSessionTs.includes('Build may finish') && computerUseSessionTs.includes('setEnabled') && uiHtml.includes('browserTargetKey') && uiHtml.includes('browserGuestCreatePromises') && uiHtml.includes('toggleComputerUse') && mainTs.includes("ipcMain.handle('agent:setComputerUseEnabled'") && mainTs.includes('computerUseState') && preloadTs.includes('setComputerUseEnabled:'), 'ComputerUse/BrowserUse: conversation-bound toggle and global occupancy survive Build completion, while each right-sidebar Browser guest is target-bound');
  ...
  assert(agentTs.includes('Memory Lab is governed by an explicit Policy chain') && agentTs.includes('prefer bounded memory_lab_query retrieval') && agentTs.includes('pass expectedUpdatedAt') && agentTs.includes('Use memory_lab_delete only for an explicit user request') && !agentTs.includes('Memory Lab stores persistent local memory for Newmark Agent') && agentTs.includes('handleMemoryLabTool') && agentTs.includes('async updateMemoryLab') && agentTs.includes('async reindexMemoryLab'), 'agent core: Memory Lab prompt exposes bounded Policy actions without injecting stored memory content');
  ...
  assert(cliCommandsTs.includes("args.includes('--msi')") && cliCommandsTs.includes('planManagedMsiInstall') && cliCommandsTs.includes('executeManagedMsiInstall') && cliCommandsTs.includes('--confirm-stop') && cliCommandsTs.includes('--confirm-remove-legacy') && cliCommandsTs.includes('--no-uninstall-previous') && cliCommandsTs.includes('--no-elevate'), 'cli commands: install-update --msi plans and executes managed MSI installs with stop/legacy confirmation flags');
  assert(installUpdateTs.includes('listRunningNewmarkProcesses') && installUpdateTs.includes('listInstalledNewmarkProducts') && installUpdateTs.includes('findLegacyNewmarkExecutables') && installUpdateTs.includes('removeLegacyNewmarkExecutables') && installUpdateTs.includes('uninstallNewmarkProduct') && installUpdateTs.includes('installMsiPackage') && installUpdateTs.includes('planManagedMsiInstall') && installUpdateTs.includes('executeManagedMsiInstall') && installUpdateTs.includes("execFileSync('where.exe'") && installUpdateTs.includes('Start-Process -FilePath "msiexec.exe"') && installUpdateTs.includes('needsStopConfirmation') && installUpdateTs.includes('needsLegacyRemovalConfirmation'), 'install update: managed MSI flow discovers running processes, installed products, and legacy executables, then stops/uninstalls/cleans with confirmation gates');
  assert(mainTs.includes("ipcMain.handle('update:checkGithub'") && mainTs.includes("ipcMain.handle('update:applyGithub'") && mainTs.includes("ipcMain.handle('update:installLocal'") && mainTs.includes("ipcMain.handle('update:planMsi'") && mainTs.includes("ipcMain.handle('update:executeMsi'"), 'main ipc: exposes GitHub, local, and managed MSI update install APIs');
  assert(electronBuilderConfigTs.includes("createDesktopShortcut: 'always'") && electronBuilderConfigTs.includes('createStartMenuShortcut: true') && packageJson.includes('"createDesktopShortcut": "always"') && packageJson.includes('"createStartMenuShortcut": true'), 'windows MSI: start-menu and desktop shortcuts are always (re)created on install and upgrade');
  ...
  assert(cfg.get('ui', 'gradient_colors') === undefined && cfg.get('ui', 'gradient_speed') === undefined && cfg.get('ui', 'gradient_width') === undefined, 'deprecated marquee customization is absent from default config');
  const legacyMarqueeRoot = path.join(TEST_DIR, 'legacy-marquee-config');
  fs.mkdirSync(legacyMarqueeRoot, { recursive: true });
  const legacyMarqueeConfig = defaultConfig();
  legacyMarqueeConfig.ui.gradient_colors = { value: ['#123456', '#abcdef'] };
  legacyMarqueeConfig.ui.gradient_speed = { value: 9 };
  legacyMarqueeConfig.ui.gradient_width = { value: 6 };
  fs.writeFileSync(path.join(legacyMarqueeRoot, 'config.json'), JSON.stringify(legacyMarqueeConfig, null, 2), 'utf-8');
  const migratedMarqueeConfig = new ConfigManager(legacyMarqueeRoot);
  const persistedMarqueeConfig = JSON.parse(fs.readFileSync(path.join(legacyMarqueeRoot, 'config.json'), 'utf-8')) as Record<string, Record<string, unknown>>;
  assert(migratedMarqueeConfig.get('ui', 'gradient_colors') === undefined
    && migratedMarqueeConfig.get('ui', 'gradient_speed') === undefined
    && migratedMarqueeConfig.get('ui', 'gradient_width') === undefined
    && !Object.prototype.hasOwnProperty.call(persistedMarqueeConfig.ui || {}, 'gradient_colors')
    && !Object.prototype.hasOwnProperty.call(persistedMarqueeConfig.ui || {}, 'gradient_speed')
    && !Object.prototype.hasOwnProperty.call(persistedMarqueeConfig.ui || {}, 'gradient_width'),
  'legacy marquee customization is removed from memory and config.json during one-time upgrade');
  ...
  bomCfg.set('general', 'default_input', 'next');
  bomCfg.save();
  const staleInputModeWorkspace = path.join(bomDir, 'workspace-stale-input-mode');
  fs.mkdirSync(staleInputModeWorkspace, { recursive: true });
  fs.writeFileSync(path.join(staleInputModeWorkspace, 'config.json'), JSON.stringify({
    general: { default_input: { value: 'guide' } },
  }), 'utf-8');
  const inputModeConfigReloaded = new ConfigManager(bomDir);
  inputModeConfigReloaded.loadWorkspaceConfig(staleInputModeWorkspace);
  assert(inputModeConfigReloaded.getStr('general', 'default_input') === 'next', 'workspace config: stale Guide cannot override the user-level Next input mode after restart');
  ...
  assert(!!agent.goal && agent.goal.checkComplete('[Goal Complete] NM_STRESS_GOAL_COMPLETE_OK'), 'checkComplete: detects an explicit bracketed completion marker');
  ...
  assert(renameApiSource.includes('publishConversationTitle(title, conversationId)')
    && renameApiSource.indexOf('this.publishConversationTitle(title, conversationId)') < renameApiSource.indexOf('private firstPersistedUserForTitleGate')
    && uiHtml.includes("String(event.type || '') === 'conversation_title'")
    && uiHtml.includes('scheduleConversationsRender();'),
  'conversation title: persisted pre-response metadata repaints the PC sidebar without entering a Build run');
  ...
  const priorInputModeWorkspaceId = inputModeReloaded.workspace.current?.id || '';
  const staleWorkspaceModeDir = fs.mkdtempSync(path.join(os.tmpdir(), 'newmark-stale-workspace-input-mode-'));
  fs.writeFileSync(path.join(staleWorkspaceModeDir, 'config.json'), JSON.stringify({ general: { default_input: { value: 'guide' } } }), 'utf-8');
  inputModeReloaded.setInputMode('next');
  const staleWorkspace = inputModeReloaded.addExternalWorkspace(staleWorkspaceModeDir);
  assert(!!staleWorkspace && inputModeReloaded.inputMode === 'next', 'inputMode: selecting a workspace with a stale Guide template preserves the user-level Next choice');
  const inputModeWorkspaceRestart = new Agent(TEST_DIR);
  assert(inputModeWorkspaceRestart.inputMode === 'next' && inputModeWorkspaceRestart.workspace.current?.path === staleWorkspaceModeDir, 'inputMode: real application restart restores Next even when the active workspace still contains Guide');
  if (priorInputModeWorkspaceId) inputModeWorkspaceRestart.selectWorkspace(priorInputModeWorkspaceId);
  if (priorInputModeWorkspaceId) inputModeReloaded.selectWorkspace(priorInputModeWorkspaceId);
  fs.rmSync(staleWorkspaceModeDir, { recursive: true, force: true });
  ...
  assert(archiveAgent.listConversationStates().some(c => c.id === 'archive-target-duplicate'), 'archiveConversation: preserves a different id even when its content exactly matches the archived target');
  ...
  conversationOrderAgent.chatMessages[0].runId = 'original-page-only';
  conversationOrderAgent.flushConversationState();
  const nestedBranch = conversationOrderAgent.branchConversation('branch-source', 0, 'nested edited request');
  assert(nestedBranch.branches.length === 3 && nestedBranch.branchGroups.length === 1 && nestedBranch.branchGroupId === branched.branchGroupId && nestedBranch.activeBranchId !== originalBranch.id && nestedBranch.workRuns.length === 0, 'conversation tree: editing the same node again appends a third sibling page to the existing pager and removes the edited message owning Build regardless of timestamps');
  const repeatedParent = nestedBranch.branches.find(branch => branch.id === originalBranch.id)!;
  const repeatedLeaf = nestedBranch.branches.find(branch => branch.id === nestedBranch.activeBranchId)!;
  const inspectedRepeatedParent = conversationOrderAgent.inspectConversationBranch('branch-source', repeatedParent.id, nestedBranch.branchGroupId);
  assert(inspectedRepeatedParent.branchGroupId === nestedBranch.branchGroupId && inspectedRepeatedParent.branches.length === 3 && inspectedRepeatedParent.branches.some(branch => branch.id === repeatedLeaf.id), 'conversation tree: inspection keeps all same-node edits in one three-page sibling group');
  const restoredNestedParent = conversationOrderAgent.switchConversationBranch('branch-source', repeatedParent.id, nestedBranch.branchGroupId);
  assert(restoredNestedParent.branchGroupId === nestedBranch.branchGroupId && restoredNestedParent.activeBranchId === repeatedParent.id, 'conversation tree: branch activation is addressed by group plus node identity');
  ...
  assert(inspectedOriginalGuide.chatMessages.some(message => message.content === 'original guide')
    && inspectedOriginalGuide.chatMessages.some(message => message.content === 'after guide')
    && inspectedOriginalGuide.workRuns[0]?.events.some(event => event.id === 'after-guide')
    && !guideBranched.chatMessages.some(message => ['original guide', 'after guide'].includes(message.content))
    && !guideBranched.workRuns[0]?.events.some(event => event.id === 'after-guide'),
  'Guide branching: the original Guide and every later message/tool event remain only on the original page and cannot leak into the edited page');
  conversationOrderAgent.switchConversationBranch('guide-branch-source', originalGuidePage.id, guideBranched.branchGroupId);
  ...
  assert(switchedForVision && modelAgent.model === 'auto' && !!modelAgent.activeModelConfig() && !modelAgent.lastRouteDecision?.excludedCandidates.some(candidate => candidate.reasons.some(reason => reason.includes('missing_capability:image_input'))), 'auto model: image input remains routable regardless of cached vision observations');
  ...
  assert(/import\s*\{[^}]*\bProxyAgent\b[^}]*\}\s*from\s*['"]undici['"]/.test(providerTransportSource)
    ...
    && agentProxySource.includes('providerProxyConfig()')
    && agentProxySource.includes("this.config.get<boolean>('proxy', 'enabled')"),
  'LLMProvider proxy: model and adapter transports use the configured HTTP/HTTPS proxy while loopback providers remain direct');
