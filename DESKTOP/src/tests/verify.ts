...
async function main() {
  ...
  assert(uiHtml.includes('function activeConversationId()') && uiHtml.includes('api.sendMessage(requestMessage, lockedConversationId)') && uiHtml.includes('composePromptRequestForSend(rawText)'), 'ui html: sends the initiating conversation id with structured text and image attachments');
  ...
  assert(!uiHtml.includes('data-tab="md-viewer"') && uiHtml.includes('id="editor-md-toggle"') && uiHtml.includes('window.toggleEditorMarkdownPreview') && uiHtml.includes("window.editorLanguageForPath(path) === 'markdown'") && uiHtml.includes('.editor-toolbar .editor-view-btn { display:none; }'), 'ui html: Markdown preview is integrated as an editor-only toggle shown only for Markdown files with sufficient CSS specificity');
  ...
  assert(uiHtml.includes('editorPredictionEnabled: true') && uiHtml.includes('window.toggleEditorPrediction') && uiHtml.includes('window.scheduleEditorCompletion') && uiHtml.includes('}, 500);') && uiHtml.includes('class="editor-ghost"') && uiHtml.includes('window.highlightEditorCode(combined, language)') && uiHtml.includes('editor-ghost-text') && uiHtml.includes("state.editorCompletionText && e.key === 'Tab'"), 'ui html: Copilot prediction defaults on, retriggers after 500ms idle, reflows highlighted code as pale insertion text, and accepts with Tab');
  ...
  assert(conversationKernelSource.includes('pendingOptions(conversationId: string): OptionQuestion[] | undefined') && mainSource.includes('conversationKernel?.pendingOptions(targetConversation) || agent.pendingOptions'), 'conversation options: state refresh reads pending questions from the requested conversation runtime instead of erasing them with shared host state');
  ...
  assert(launcherTs.includes('function pathArgValue') && launcherTs.includes('function userRuntimeRoot') && launcherTs.includes("path.join(os.homedir(), '.Newmark')") && launcherTs.includes('function legacyUserDataRoot') && launcherTs.includes('function migrateLegacyRuntimeRoot') && launcherTs.includes('function writableRuntimeRoot') && launcherTs.includes("path.join(userRuntimeRoot(), 'Roots'") && launcherTs.includes("const explicitRoot = pathArgValue(args, '--root')") && launcherTs.includes('const root = explicitRoot ? writableRuntimeRoot(explicitRoot) : userRuntimeRoot()'), 'launcher entrypoint: CLI runtime config/state defaults to user home .Newmark and remaps protected or unwritable explicit roots below .Newmark Roots');
  assert(mainTs.includes('function userRuntimeRoot') && mainTs.includes("path.join(os.homedir(), '.Newmark')") && mainTs.includes('function legacyUserDataRoot') && mainTs.includes('function migrateLegacyRuntimeRoot') && mainTs.includes('function canWriteDirectory') && mainTs.includes('function isProtectedInstallRoot') && mainTs.includes('function shadowRootFor') && mainTs.includes('function writableRuntimeRoot') && mainTs.includes("process.env.ProgramFiles") && mainTs.includes("path.join(userRuntimeRoot(), 'Roots'") && mainTs.includes('if (explicitRoot) return writableRuntimeRoot(explicitRoot)') && mainTs.includes('return getRoot();') && mainTs.includes('firstRunInit(root);') && mainTs.includes('firstRunInit(fallbackRoot)') && mainTs.includes('logStartupFailure(`firstRunInit:${root}`'), 'main startup: packaged runtime config/state defaults to user home .Newmark, migrates legacy userData state, and remaps protected explicit roots below .Newmark Roots');
  ...
  assert(workspaceTs.includes('normalizeInternalWorkspace') && workspaceTs.includes("const expectedPath = path.join(this.rootPath, 'Work', name)") && workspaceTs.includes('if (internalChanged) this.saveInternal();') && workspaceTs.includes('if (!stateCurrent?.path || path.resolve(stateCurrent.path) !== path.resolve(stored.path)) this.saveState();'), 'workspace migration: internal workspace absolute paths are normalized under the current runtime root after .Newmark/userData migration');
  ...
  assert(uiHtml.includes('id="title-app-logo"') && uiHtml.includes('id="title-app-icon"') && uiHtml.includes("systemColorScheme.addEventListener('change', window.refreshTitlebarSystemIcon)") && uiHtml.includes("systemColorScheme.matches ? '../../assets/app-icon-light.png' : '../../assets/app-icon-dark.png'"), 'app icons: custom titlebar icon follows live system color-scheme changes with transparent light/dark assets');
  ...
  assert(distPortableScript.includes('verifyReleaseCliSmoke()') && distPortableScript.includes('release-cli-smoke.cjs'), 'dist windows release: runs release CLI smoke after packaging');
  ...
  assert(inspectToolContent.some(part => part.type === 'image_url' && String(part.image_url?.url || '').startsWith('data:image/png;base64,')), 'image_inspect kernel: returns the crop as structured vision content');
  assert(inspectToolContent.some(part => part.type === 'text' && String(part.text || '').includes('"crop"')) && !JSON.stringify(inspectToolContent.filter(part => part.type === 'text')).includes('image_data_url'), 'image_inspect kernel: exposes crop metadata without leaking base64 into visible tool text');
