...
function runWindowsWslBuild() {
  ...
  const script = [
    ...
    'trap cleanup EXIT',
    'mkdir -p "$build_root/repo/DESKTOP" "$build_root/repo/TUI" "$build_root/repo/android" "$build_root/repo/release"',
    ...
    `rsync -a --delete --exclude='build/' --exclude='.gradle/' --exclude='isolated-release-*/' --exclude='APK/' ${wslQuote(`${wslRepoRoot}/android/`)} "$build_root/repo/android/"`,
