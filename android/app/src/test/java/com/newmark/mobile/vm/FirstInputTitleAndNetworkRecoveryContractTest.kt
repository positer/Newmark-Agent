...
class FirstInputTitleAndNetworkRecoveryContractTest {
    ...
    fun firstInputUsesOneIndependentTitleRequestWithoutDirectCopy() {
        ...
        val titleRequestSource = File("src/main/java/com/newmark/mobile/vm/FirstInputTitleRequest.kt").readText()
        val titleApplicationSource = source
            .substringAfter("private suspend fun requestFirstInputConversationTitle(")
        ...
        assertTrue(titleRequestSource.contains("if (imageAttachmentCount > 0 && title.isBlank())"))
        assertTrue(titleRequestSource.contains("current.titleRequestMessageId != messageId"))
        ...
        assertTrue(compactSource.contains("requestFirstInputConversationTitle( targetConversationId, firstTitleMessageId, titleInput, titleConfig, turnIntelligence, turnThinkingTierMap, imageAttachmentCount = firstUser?.imageAttachments?.size?.takeIf { it > 0 } ?: images.size, onFailure = { lastTitleFailure = it }, )"))
        assertTrue(titleApplicationSource.contains("requestAndApplyFirstInputTitle("))
        assertTrue(titleApplicationSource.contains("conversationPersistence.commitTitle("))
        assertTrue(source.contains("withContext(Dispatchers.IO) { conversationStore.save(snapshot) }"))
        assertTrue(titleRequestSource.contains("applyTitle: suspend (String) -> Result<Boolean>"))
        assertTrue(titleRequestSource.contains("return persist(updated).map"))
        assertTrue(source.contains("error = com.newmark.mobile.data.CONVERSATION_SAVE_FAILURE_MESSAGE"))
        assertFalse(titleApplicationSource.contains("asyncRuntime"))
        assertFalse(titleRequestSource.contains("withTimeout"))
