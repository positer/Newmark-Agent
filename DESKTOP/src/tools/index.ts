...
export class ToolExecutor {
  ...
  definitions(mode?: string): unknown[] {
    ...
    const tools = [
      ...
      t('subagent_send', 'Send selected content from your own history to a same-conversation peer, or id="root" for the main agent. wakeup defaults to false: an inactive receiver only stores the message; true requests activation. An active receiver handles either value normally. With no content/message, send the last visible text-history entry. Choose each explicit content kind and its range; summaries are written by you, not generated by the tool. Existing receiver history and static request metadata are unchanged until receipt.', {
        id: { type: 'string', description: 'Exact peer id, or root for the main agent.' },
        name: { type: 'string', description: 'Convenience peer name.' },
        wakeup: { type: 'boolean', default: false, description: 'Activate an inactive receiver when true; false only stores mail. Both deliver normally to an active receiver.' },
        content: { type: 'array', minItems: 1, maxItems: 16, description: 'Ordered content selections. Each selection requires a range. Do not combine with message/prompt.', items: {
          type: 'object', required: ['kind', 'range'], additionalProperties: false, properties: {
            kind: { type: 'string', enum: ['summary', 'text_history', 'tool_history'] },
            range: { type: 'object', additionalProperties: false, description: 'Use last:N, or the 1-based inclusive start/end range in the selected kind. Text entries exclude hidden input; tool entries are complete call/result pairs; summary uses the combined visible timeline.', properties: {
              last: { type: 'integer', minimum: 1 }, start: { type: 'integer', minimum: 1 }, end: { type: 'integer', minimum: 1 },
            } },
            text: { type: 'string', description: 'Your concise summary of the selected source range, required only for kind=summary.' },
          },
        } },
        message: { type: 'string', description: 'Explicit free text, for a new directive or correction. Cannot be combined with content.' },
        prompt: { type: 'string', description: 'Legacy alias for message.' },
        kind: { type: 'string', enum: ['directive', 'question', 'result', 'handoff'] }, reply_to: { type: 'string' }, correlation_id: { type: 'string' },
      }, []),
      ...
      switch (tool) {
        ...
        case 'computer_use': {
          ...
          const output = await runComputerUse({
            ...
            captureMaxWidth: Number((args as Record<string, unknown>).capture_max_width),
            captureMaxHeight: Number((args as Record<string, unknown>).capture_max_height),
            gradientColors: Array.isArray((args as Record<string, unknown>).gradient_colors) ? (args as Record<string, unknown>).gradient_colors as string[] : undefined,
            gradientSpeed: (args as Record<string, unknown>).gradient_speed !== undefined ? Number((args as Record<string, unknown>).gradient_speed) : undefined,
            gradientWidth: (args as Record<string, unknown>).gradient_width !== undefined ? Number((args as Record<string, unknown>).gradient_width) : undefined,
            invocation: context.invocation,
            ownerId: owner,
