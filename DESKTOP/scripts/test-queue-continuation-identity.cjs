...
async function run(options = {}) {
  ...
  const cases = ['delete-second', 'edit-first', 'edit-second', 'reorder', 'guide-deferred', 'drain-duplicates', 'paused-resume'];
  for (const scenario of cases) {
    ...
    try {
      ...
      if (scenario === 'reorder') kernel.queueAction(target, 'reorder', { orderedIds: ['second', 'middle', 'first'] });
      ...
      const expected = rows.map(item => item.text);
      if (scenario === 'guide-deferred') expected.push('same');
