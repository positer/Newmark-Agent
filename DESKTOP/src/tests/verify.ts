...
async function main() {
  ...
  assert(uiHtml.includes('function renderProvisionalEditedBranch(message, messageIndex, editedText, target, editingGuide)') && uiHtml.includes("if (!editingGuide) return [];") && uiHtml.includes('copiedRun.events = guideIndex >= 0 ? events.slice(0, guideIndex)') && uiHtml.includes("type: 'guide_applied'") && uiHtml.includes('state.renderedChatMessages = Array.isArray(messages)'), 'ui html: provisional message edits remove the owning Build immediately while Guide edits preserve only the owning Build prefix and replace the Guide node in place');
