...
Download published packages from the [GitHub releases page](https://github.com/positer/Newmark-Agent/releases). The source now targets the next `dev-0.3.13` candidate; published artifacts remain at the last verified `dev-0.3.12` baseline until the new package gate completes.
...
- `Newmark-Agent-0.3.13-x64.msi`: per-machine Windows installer for the next candidate build.
- `Newmark-Agent-0.3.13-win-unpacked-x64.zip`: portable Windows directory with GUI, TUI, CLI, and launcher files for the next candidate build.

### Linux

- `Newmark-Agent-0.3.13-x86_64.AppImage`: portable Linux desktop package for the next candidate build.
- `Newmark-Agent-0.3.13-amd64.deb`: Debian/Ubuntu package for the next candidate build.
- `Newmark-Agent-0.3.13-linux-unpacked-x64.zip`: unpacked Linux distribution for the next candidate build.
