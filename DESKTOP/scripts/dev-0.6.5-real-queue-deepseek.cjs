...
const archiveDir = path.join(desktopRoot, '..', 'archive', '20260915-dev065-real-deepseek-queue');
...
(async () => {
  const installedExe = String(process.env.NEWMARK_VISUAL_EXE || '');
  ...
  const providers = config.models.providers.value || config.models.providers;
  const provider = providers.find(p => p.id === 'provider-deepseek-001');
  if (!provider) throw new Error('Configured DeepSeek provider missing');
  fs.writeFileSync(path.join(root, 'config.json'), JSON.stringify({models:{providers:[provider],default_model:MODEL,auto_switch:false,fallback_on_unavailable:false},general:{language:'zh'},proxy:config.proxy}));
  ...
  try {
    ...
    evidence.steps.push({ name: 'selectModel', value: await evaluate(cdp, `(async () => {
      ...
      if (state.model !== ${JSON.stringify(MODEL)}) throw new Error('Wrong visible deployment: ' + state.model);
    ...
    evidence.steps.push({ name: 'firstBuild', value: await evaluate(cdp, `(async () => {
      ...
      window.__dev065Send = window.sendMessage('build').catch(error => { window.__dev065SendError = String(error.message || error); });
    ...
    evidence.passed = !!queued && !!final && !final.running && (final.queueItems || []).length === 0
      && (final.renderedUserBubbles || []).some(text => String(text).includes('排队测试'))
      && (final.chatMessages || []).some(line => String(line).includes('QUEUED_TURN_OK'));
    if (!evidence.passed) process.exitCode = 1;
    evidence.drainShot = await shot(cdp, 'drained-real-model.png');
    fs.writeFileSync(path.join(archiveDir, 'evidence.json'), JSON.stringify(evidence, null, 2));
