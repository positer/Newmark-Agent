...
(async()=>{
  ...
  try{
    ...
    const toolbar=await evalJs(cdp,`(() => { const buttons=Array.from(document.querySelectorAll('.editor-toolbar .et-btn')); const visible=buttons.filter(b=>getComputedStyle(b).display!=='none'); const copilot=document.querySelector('#editor-copilot-toggle'); return {count:buttons.length,visible:visible.length,sizes:visible.map(b=>{const br=b.getBoundingClientRect(),ir=b.querySelector('.nm-icon').getBoundingClientRect();return {w:br.width,h:br.height,text:b.textContent.trim(),iconW:ir.width,iconH:ir.height,contained:ir.left>=br.left&&ir.top>=br.top&&ir.right<=br.right&&ir.bottom<=br.bottom};}),copilotActive:copilot.classList.contains('active'),pressed:copilot.getAttribute('aria-pressed'),highlight:document.querySelector('#editor-highlight').innerHTML}; })()`);
    if(toolbar.count!==4||toolbar.visible!==3||toolbar.sizes.some(item=>item.w!==30||item.h!==30||item.text||item.iconW!==15||item.iconH!==15||!item.contained)||!toolbar.copilotActive||toolbar.pressed!=='true'||!toolbar.highlight.includes('tok-keyword')||!toolbar.highlight.includes('tok-type')||toolbar.highlight.includes('<span <span'))fail(`toolbar/highlight failed: ${JSON.stringify(toolbar)}`);
    ...
    const moved=await evalJs(cdp,`(() => { const ta=document.querySelector('#editor-textarea'); const next=Math.min(ta.value.length,ta.selectionStart+4); ta.setSelectionRange(next,next); window.handleEditorCaretChange(); return {text:window.state.editorCompletionText,anchor:window.state.editorCompletionAnchor,timer:!!window.state.editorCompletionTimer,ghost:document.querySelector('#editor-ghost').textContent}; })()`);
    if(moved.text||moved.anchor||!moved.timer||moved.ghost)fail(`caret movement did not invalidate and restart completion: ${JSON.stringify(moved)}`);
