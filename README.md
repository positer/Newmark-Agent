...
The `dev-0.3.12` safe product gates pass the temporary-root, rebuilt Windows-package, protected-install-shape, and expanded GUI/TUI/CLI scopes, but the latest independent no-context release-flow audit is `FAIL/HOLD`. It found post-package runtime contamination in the canonical `win-unpacked` directory, a release validation script whose `--help` can launch the stale Program Files path, a GUI exit-code gate hazard, and an invalid-model validation result of `[]` with exit `0`. The latest package was not UAC-installed and the existing `C:\Program Files\Newmark Agent` copy is stale. No-provider and unavailable explicit-model requests fail closed in the rebuilt source/package gate, while default `validate-models` loads fresh durable evidence read-only and never writes config unless `--persist` is explicit. The dedicated `BLACKBOX_GUI_NO_MODEL` GUI gate proves IPC failure, renderer runtime, WorkRun, and visible diagnostic all settle as terminal error with no synthetic completion. Full release-flow adjudication is [`archive/20260812-1630-dev-0.3.12-blackbox-release-flow-adjudication.md`](archive/20260812-1630-dev-0.3.12-blackbox-release-flow-adjudication.md); the delegated raw report is [`archive/20260812-1610-dev-0.3.12-blackbox-release-report.md`](archive/20260812-1610-dev-0.3.12-blackbox-release-report.md).
