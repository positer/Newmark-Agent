...
async function main() {
  ...
  assert(uiHtml.includes('async function stopTargetConversationForMessageEdit(target)') && uiHtml.includes("api.stopConversation({ target: target, runId: runId, force: false })") && uiHtml.includes("api.stopConversation({ target: target, runId: runId, force: true })") && uiHtml.includes('if (!await stopTargetConversationForMessageEdit(target)) return;') && uiHtml.includes("delete state.activeSendCallsByTarget[runtimeKey]") && uiHtml.includes('optimisticMessageIndex: Number(messageIndex)') && uiHtml.includes('renderConversationBranchPagers();'), 'ui html: editing stops only the selected conversation runtime, enters the new page, and immediately paginates the edited first message before its Build completes');
