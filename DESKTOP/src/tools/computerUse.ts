...
function startTakeoverOverlay(durationMs = 0, input: { colors?: string[]; speed?: number; width?: number } = {}): Record<string, unknown> {
  ...
  const script = [
    ...
    '  $step = [Math]::Max(1, [int][Math]::Min(6, [Math]::Max(1, $script:thick)))',
    '  for ($x = 0; $x -lt $w; $x += $step) {',
    '    $rw = [Math]::Min($step, $w - $x)',
    '    $pos = (($x / $perimeter) * $count) + $script:phase',
    '    $color = Newmark-LerpColor $pos',
    '    $brush = New-Object System.Drawing.SolidBrush($color)',
    '    try { $g.FillRectangle($brush, $x, 0, $rw, $script:thick) } finally { $brush.Dispose() }',
    '    $pos = (((($w + $h) + ($w - $x)) / $perimeter) * $count) + $script:phase',
    '    $color = Newmark-LerpColor $pos',
    '    $brush = New-Object System.Drawing.SolidBrush($color)',
    ...
    '$timer.Interval = [Math]::Max(16, [int](1000 * $speedSeconds / 48))',
    '$timer.Add_Tick({',
    '  $script:phase = ($script:phase + 1) % $script:colors.Count',
    '  $script:form.Invalidate()',
    '})',
    '$timer.Start()',
